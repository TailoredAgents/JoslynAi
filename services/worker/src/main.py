...
MAX_JOB_RETRIES = int(os.getenv("JOB_MAX_RETRIES", "3"))
...
@register_job("ingest_pdf")
def handle_ingest_pdf(task: Dict[str, Any]) -> None:
    job_id = task.get("job_id")
    org_id = task.get("org_id")
    try:
        from src.index import _patch_job as patch_job  # reuse helper
    except Exception:
        patch_job = lambda *args, **kwargs: None  # noqa: E731

    _patch_job(job_id, "ocr", "processing", org_id)
    task = process_pdf(task)
    _patch_job(job_id, "ocr", "done", org_id)
    ...
    _patch_job(job_id, "index", "processing", org_id)
    embed_and_store(task)
    _patch_job(job_id, "index", "done", org_id)

    if doc_type_final and isinstance(doc_type_final, str) and "eob" in doc_type_final.lower():
        _patch_job(job_id, "extract", "processing", org_id)
        extract_eob(task)
        _patch_job(job_id, "extract", "done", org_id)
...
def handle_extract_iep_job(task: Dict[str, Any]) -> None:
    extract_iep(task)


@register_job("extract_eob")
def handle_extract_eob_job(task: Dict[str, Any]) -> None:
    extract_eob(task)


@register_job("prep_iep_diff")
def handle_prep_iep_diff(task: Dict[str, Any]) -> None:
    db_url = os.getenv("DATABASE_URL")
    if not db_url:
        return
...
@register_job("denial_explain")
def handle_denial_explain(task: Dict[str, Any]) -> None:
    db_url = os.getenv("DATABASE_URL")
    if not db_url:
        return
...
@register_job("research_summary")
def handle_research_summary(task: Dict[str, Any]) -> None:
    db_url = os.getenv("DATABASE_URL")
    if not db_url:
        return
    document_id = task.get("document_id")
...
@register_job("build_advocacy_outline")
def handle_build_advocacy_outline(task: Dict[str, Any]) -> None:
    db_url = os.getenv("DATABASE_URL")
    if not db_url:
        return
...
@register_job("seed_safety_phrases")
def handle_seed_safety_phrases(task: Dict[str, Any]) -> None:
    db_url = os.getenv("DATABASE_URL")
    if not db_url:
        return
...
@register_job("generate_safety_phrase")
def handle_generate_safety_phrase(task: Dict[str, Any]) -> None:
    db_url = os.getenv("DATABASE_URL")
    if not db_url:
        return
    child_id = task.get("child_id")
...
def handle_build_one_pager(task: Dict[str, Any]) -> None:
    db_url = os.getenv("DATABASE_URL")
    if not db_url:
        return
    ...
    try:
        with psycopg.connect(db_url) as conn:
            ...
            prompt_parts.append("Sensory supports:")
            for item in sensory:
                prompt_parts.append(f"- {item}")
            prompt_parts.append("Communication preferences:")
            for item in communications:
                prompt_parts.append(f"- {item}")
            prompt_parts.append("Recommendations:")
            for item in recommendations or []:
                prompt_parts.append(f"- {item.get('title')}: {item.get('recommendation')}")
            if segments:
                prompt_parts.append("Document excerpts (cite labels):")
                for seg in segments:
                    prompt_parts.append(f"[{seg['label']}] {seg['doc_name']} (p.{seg['page']})\n{seg['text']}")
            prompt = "\n\n".join(prompt_parts)
...
@register_job("goal_smart")
def handle_goal_smart(task: Dict[str, Any]) -> None:
    db_url = os.getenv("DATABASE_URL")
    if not db_url:
        return
    child_id = task.get("child_id")
...
def handle_build_appeal_kit(task: Dict[str, Any]) -> None:
...
@register_job("prep_recommendations")
def handle_prep_recommendations(task: Dict[str, Any]) -> None:
    db_url = os.getenv("DATABASE_URL")
    if not db_url:
        return
    child_id = task.get("child_id")
