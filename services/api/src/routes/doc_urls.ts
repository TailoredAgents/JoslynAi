...
export default async function routes(app: FastifyInstance) {
  app.get<{ Params: { id: string }, Querystring: { ttl?: string } }>(
    ...
    async (req, reply) => {
      ...
      const doc = await (prisma as any).documents.findUnique({ where: { id }, select: { storage_uri: true } });
  ...
  app.get<{ Params: { id: string } }>("/letters/:id/url", async (req, reply) => {
    const row = await (prisma as any).letters.findUnique({ where: { id: (req.params as any).id }, select: { pdf_uri: true } });
