...
export default async function routes(app: FastifyInstance) {
  ...
  app.post("/next-best-steps/generate", async (req, reply) => {
    ...
    const soon = await (prisma as any).deadlines.findMany({ where: { org_id: orgId, child_id, due_date: { lte: new Date(now + 14*86400000), gte: new Date(now) } } });
    for (const d of soon) {
      ...
      const row = await (prisma as any).next_best_steps.upsert({
        ...
        create: { org_id: orgId, child_id, kind: "deadline_upcoming", payload_json: payload, dedupe_key: key, suggested_at: new Date(), expires_at: new Date(now + 14*86400000) },
    ...
    if (hasDenial) {
      ...
      const row = await (prisma as any).next_best_steps.upsert({
        ...
        create: { org_id: orgId, child_id, kind: "appeal_recommended", payload_json: payload, dedupe_key: key, suggested_at: new Date() },
